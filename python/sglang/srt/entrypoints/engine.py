...
from sglang.srt.bullet.launchers import launch_msg_router
...
class Engine(EngineBase):
    ...
    def init_weights_update_group(
        ...
        loop = asyncio.get_event_loop()
        return loop.run_until_complete(self.tokenizer_manager.init_weights_update_group(obj, None))
    ...
    def update_weights_from_distributed(
        ...
        loop = asyncio.get_event_loop()
        return loop.run_until_complete(self.tokenizer_manager.update_weights_from_distributed(obj, None))
    ...
    def update_weights_from_tensor(
        ...
        loop = asyncio.get_event_loop()
        return loop.run_until_complete(self.tokenizer_manager.update_weights_from_tensor(obj, None))
    ...
    def update_weights_from_disk(
        ...
        loop = asyncio.get_event_loop()
        return loop.run_until_complete(self.tokenizer_manager.update_weights_from_disk(obj, None))
...
def _launch_subprocesses(
    ...
    if server_args.msg_chat_ipc_name is None:
        server_args.msg_chat_ipc_name = port_args.msg_chat_ipc_name
    if server_args.reqrpc_ipc_name is None:
        server_args.reqrpc_ipc_name = port_args.reqrpc_ipc_name

    decode_nccl = PortArgs.init_new(server_args).nccl_port
    decode_port_args = dataclasses.replace(port_args, nccl_port=decode_nccl)
    decode_server_args = dataclasses.replace(server_args, is_bullet_decode=True, is_bullet_prefill=False)

    # If using model from www.modelscope.cn, first download the model.
    server_args.model_path, server_args.tokenizer_path = prepare_model_and_tokenizer(
        server_args.model_path, server_args.tokenizer_path
    )
    msg_server = launch_msg_router(port_args.msg_chat_ipc_name)
    msg_driver = MsgClient(0, 0, name=IPCGroup.DRIVER, ipc_name=port_args.msg_chat_ipc_name)
    logger.info("Message router is launched")
    ...
    if server_args.dp_size == 1:
        ...
        many_server_args = [server_args, decode_server_args]
        many_port_args = [port_args, decode_port_args]
        loops = 1
        if server_args.enable_bullet_engine:
            loops = 2
        ...
        for pp_rank in pp_rank_range:
            for tp_rank in tp_rank_range:
                for loop in range(loops):
                    tmp_server_args = many_server_args[loop]
                    tmp_port_args = many_port_args[loop]
                    ...
                    os.environ["CUDA_MPS_PIPE_DIRECTORY"] = server_args.mps_pipe_dir
                    proc = mp.Process(
                    ...
                    logger.info(f"Spawning TP {tp_rank} PP {pp_rank} scheduler process {proc.pid} on GPU {gpu_id}")
                    if is_hip():
                        time.sleep(5)
    else:
    ...
    if server_args.enable_bullet_engine:
        logger.info("Scheduler processes are spawning")
        client_ids = [
            IPCGroup.engine_id(server_args.host, server_args.port, i, j, prefill)
            for i in range(tp_size_per_node)
            for j in range(pp_size_per_node)
            for prefill in [True] + [False] * (loops - 1)
        ]

        # wait for engines to load the remote model and ready to init kv cache
        for i in range(len(client_ids)):
            logger.info(f"Receiving READY_TO_INIT_KV... {i}th")
            msg_driver.recv_many([MsgTy.READY_TO_INIT_KV])

        logger.info("All engines are ready to init kv cache, sending SPAWN_KV to master")
        for i in range(len(client_ids) // 2):
            msg_driver.send(client_ids[i * 2], MsgTy.SPAWN_KV)
        # msg_driver.recv(MsgTy.KV_MEMPOOL_INITED)

        # logger.info(f"KV memory pool is initialized, sending GRANT_KV_INIT to engines {client_ids}")
        # time.sleep(2)
        # msg_driver.send(client_ids, MsgTy.GRANT_KV_INIT)
