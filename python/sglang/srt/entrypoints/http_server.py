...
from sglang.srt.bullet.list_mps_proc import list_mps_clients
...
##### Bullet API endpoints #####


@app.get("/backend_type")
async def backend_type() -> Response:
    return PlainTextResponse(content=_global_state.backend, status_code=200)


@app.post("/rpc_call")
async def rpc_call(request: Request) -> Response:
    body = await request.json()
    method = body.get("method")
    parameters = body.get("parameters", {})
    barrier = body.get("barrier", True)
    if not method:
        return ORJSONResponse({"error": "Missing 'method' field"}, status_code=HTTPStatus.BAD_REQUEST)
    ret = await _global_state.tokenizer_manager.rpc_call(
        RpcReqInput(method=method, parameters=parameters, barrier=barrier)
    )
    return Response(
        content=ret.message,
...
async def health_generate(request: Request) -> Response:
    ...
    if _global_state.tokenizer_manager.is_image_gen:
        ...
        gri = EmbeddingReqInput(rid=rid, input_ids=[0], sampling_params=sampling_params, log_metrics=False)
...
async def get_server_info():
    ...
    internal_states: List[Dict[Any, Any]] = await _global_state.tokenizer_manager.get_internal_state()
    return {
...
async def update_weights_from_disk(obj: UpdateWeightFromDiskReqInput, request: Request):
    ...
    success, message, num_paused_requests = await _global_state.tokenizer_manager.update_weights_from_disk(
        obj, request
    )
    content = {
        "success": success,
...
async def init_weights_update_group(obj: InitWeightsUpdateGroupReqInput, request: Request):
    """Initialize the parameter update group."""
    success, message = await _global_state.tokenizer_manager.init_weights_update_group(obj, request)
...
async def update_weights_from_distributed(obj: UpdateWeightsFromDistributedReqInput, request: Request):
    """Update model parameter from distributed online."""
    success, message = await _global_state.tokenizer_manager.update_weights_from_distributed(obj, request)
    content = {"success": success, "message": message}
    if success:
        return ORJSONResponse(content, status_code=200)
    else:
...
async def open_session(obj: OpenSessionReqInput, request: Request):
    ...
    try:
        ...
        if session_id is None:
            raise Exception("Failed to open the session. Check if a session with the same id is still open.")
        return session_id
    except Exception as e:
        return _create_error_response(e)
...
async def openai_v1_completions(request: CompletionRequest, raw_request: Request):
    ...
    return await raw_request.app.state.openai_serving_completion.handle_request(request, raw_request)
...
def launch_server(
    ...
    """
    tokenizer_manager, template_manager, scheduler_info = _launch_subprocesses(server_args=server_args)
    set_global_state(
        _GlobalState(
            tokenizer_manager=tokenizer_manager,
            template_manager=template_manager,
            scheduler_info=scheduler_info,
            backend="bullet" if server_args.enable_bullet_engine else f"sglang_{server_args.chunked_prefill_size}",
        )
    )
    ...
    if tokenizer_manager.image_token_id is not None and not server_args.skip_tokenizer_init:
        image_token_text = tokenizer_manager.tokenizer.decode([tokenizer_manager.image_token_id])
...
def _execute_server_warmup(
    ...
    # Test if connected to MPS
    if server_args.enable_bullet_engine:
        clients = list_mps_clients(
            {
                "CUDA_MPS_PIPE_DIRECTORY": server_args.mps_pipe_dir,
            }
        )
        if len(clients) == 0:
            logger.error(f"Bullet cannot connect to MPS: CUDA_MPS_PIPE_DIRECTORY={server_args.mps_pipe_dir}")
            if not server_args.disable_no_mps_crash:
                logger.error(
                    f"Crashing due to MPS is not enabled and server_args.disable_no_mps_crash is False"
                )
                kill_process_tree(os.getpid())
    ...
    try:
        if server_args.disaggregation_mode == "null":
            ...
            logger.info(f"End of prefill warmup with status {res.status_code}, resp: {res.json()}")
