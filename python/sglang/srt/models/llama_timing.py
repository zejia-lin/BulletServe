...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        with GLOBAL_TIMER.time_operation("gate_up_proj", x):
            gate_up, _ = self.gate_up_proj(x)
        x = self.act_fn(gate_up)
        with GLOBAL_TIMER.time_operation("down_proj", x):
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        with GLOBAL_TIMER.time_operation("qkv_proj", hidden_states):
            qkv, _ = self.qkv_proj(hidden_states)
        q, k, v = qkv.split([self.q_size, self.kv_size, self.kv_size], dim=-1)
        q, k = self.rotary_emb(positions, q, k)
        with GLOBAL_TIMER.time_operation("attn", q):
            attn_output = self.attn(q, k, v, forward_batch)
        with GLOBAL_TIMER.time_operation("o_proj", attn_output):
    ...
    def load_kv_cache_scales(self, quantization_param_path: str) -> None:
        ...
        for layer_idx, scaling_factor in kv_cache_scales_loader(
            ...
            if hasattr(layer_self_attn.attn, "k_scale"):
                ...
                raise RuntimeError("Self attention has no KV cache scaling " "factor attribute!")


class LlamaForCausalLMTiming(nn.Module):
    ...
    def forward(
        ...
        if not get_embedding:
            with GLOBAL_TIMER.time_operation("lm_head", hidden_states, is_last_op=True):
    ...
    def get_weights_by_name(
        ...
        try:
            if name == "lm_head.weight" and self.config.tie_word_embeddings:
                ...
                return self.model.embed_tokens.weight.cpu().to(torch.float32).numpy().tolist()[:truncate_size]
...
class Phi3ForCausalLM(LlamaForCausalLMTiming):
    pass


class InternLM3ForCausalLM(LlamaForCausalLMTiming):
    pass


EntryClass = [LlamaForCausalLMTiming]
