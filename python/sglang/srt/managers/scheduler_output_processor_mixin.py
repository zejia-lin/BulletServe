...
_bg_forever_loop_counter = 0
_BG_FOREVER_LOOP_START = 8
...
class SchedulerOutputProcessorMixin:
    ...
    def process_batch_result_prefill(
        ...
        if self.is_generation:
            ...
            #     global _bg_forever_loop_counter
            #     if _bg_forever_loop_counter < _BG_FOREVER_LOOP_START:
            #         _bg_forever_loop_counter += 1
            #     else:
            #         batch.reqs, next_token_ids = [], []
    ...
    def process_batch_result_decode(
        ...
        # logger.info(f"@@@ is forver loop: {self.server_args.is_forever_bg_loop}, bs={len(batch.reqs)} "
        #             f"outlens={sum(len(req.output_ids) for req in batch.reqs)}")
        if self.server_args.is_forever_bg_loop:
            global _bg_forever_loop_counter
            if _bg_forever_loop_counter < _BG_FOREVER_LOOP_START:
                _bg_forever_loop_counter += 1
                self.reqs_backup = batch.reqs
            else:
    ...
    def stream_output_generation(
        ...
        # logger.info(f"Start stream output of {reqs}, len={len(reqs[0].output_ids)}")
        rids = []
        ...
        if rids:
            if self.model_config.is_multimodal_gen:
                return
            # logger.info(f"Stream output to detokenizer: {rids}")
