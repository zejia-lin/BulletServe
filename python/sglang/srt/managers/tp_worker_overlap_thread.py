...
from sglang.srt.bullet.observability import PredictorInfoEntry
from sglang.srt.bullet.shared_mng import SelectedSMPolicy
...
class TpModelWorkerClient:
    ...
    def __init__(
        ...
        # Load the model
        self.worker = TpModelWorker(server_args, gpu_id, tp_rank, pp_rank, dp_rank, nccl_port)
        ...
        self.forward_stream = self.worker.forward_stream
    ...
    def forward_thread_func(self):
        try:
            if self.worker.server_args.enable_bullet_engine:
                self.forward_thread_loop_bullet()
            else:
                self.forward_thread_loop()
        except Exception:
    ...
    @DynamicGradMode()
    def forward_thread_loop_bullet(self):
        batch_pt = 0
        batch_lists = [None] * 2
        ...
        while True:
            ...
            entry = self.worker.update_bullet_before_forward(
                model_worker_batch, create_entry=self.worker.server_args.enable_record_timing
            )
            with torch.get_device_module(self.device).stream(self.forward_stream):
                self.forward_thread_func_inner(model_worker_batch, future_token_ids_ct, entry)

    @DynamicGradMode()
    def forward_thread_func_inner(
        self, model_worker_batch: ModelWorkerBatch, future_token_ids_ct, entry: PredictorInfoEntry
    ):
        ...
        # Run forward
        if model_worker_batch.forward_mode.is_prefill() and self.worker.server_args.num_prefill_layers > 0:
            forward_batch = ForwardBatch.init_new(model_worker_batch, self.worker.model_runner)
            pp_proxy_tensors = None
            for step in self.worker.layerwise_prefill_step_generator():
                self.worker.update_bullet_before_forward(model_worker_batch, create_entry=False)
                logits_output, next_token_ids, can_run_cuda_graph = self.worker.forward_batch_generation(
                    model_worker_batch,
        ...
        self.future_token_ids_map[future_token_ids_ct + 1 : future_token_ids_ct + bs + 1] = next_token_ids

        # Copy results to the CPU
        if model_worker_batch.return_logprob:
            logits_output.next_token_logprobs = logits_output.next_token_logprobs.to("cpu", non_blocking=True)
        ...
        self.output_queue.put((copy_done, logits_output, next_token_ids, can_run_cuda_graph, entry))

    def resolve_last_batch_result(self, launch_done: Optional[threading.Event] = None):
        copy_done, logits_output, next_token_ids, can_run_cuda_graph, entry = self.output_queue.get()
        if launch_done is not None:
            launch_done.wait()
        copy_done.synchronize()

        self.worker.update_bullet_after_forward(entry)
    ...
    def forward_thread_loop(self):
        ...
        while True:
            ...
            with torch.get_device_module(self.device).stream(self.forward_stream):
                self.forward_thread_func_inner(
                    model_worker_batch, future_token_ids_ct, SelectedSMPolicy.DEFAULT
                )
    ...
    def dump_predictor_infos(self, path: str, clear: bool):
        return self.worker.dump_predictor_infos(path, clear)
