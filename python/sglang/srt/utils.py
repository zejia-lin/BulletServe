...
def maybe_remove_weight(module: torch.nn.Module) -> torch.nn.Module:
    # logger.info(f"Remove weight is {REMOVE_WEIGHT}")
    if REMOVE_WEIGHT:
        logger.info(f"Removing weights")
...
def maybe_offload_to_cpu(module: torch.nn.Module) -> torch.nn.Module:
    ...
    module = maybe_remove_weight(module)
...
def configure_logger(server_args, prefix: str = ""):
    if SGLANG_LOGGING_CONFIG_PATH := os.getenv("SGLANG_LOGGING_CONFIG_PATH"):
        ...
        return
    format = f"[%(levelname)s %(asctime)s.%(msecs)03d %(filename)s:%(lineno)d{prefix} %(process)d] %(message)s"
    # format = f"[%(asctime)s{prefix}] %(message)s"
    # format = f"[%(asctime)s.%(msecs)03d{prefix}] %(message)s"
    level = server_args.upper() if isinstance(server_args, str) else server_args.log_level
    logging.basicConfig(
